import {BACKEND_API_ENDPOINT} from "./keys";
...
async function handle(request) {
    ...
    const signature = await sign(key, timestamp);

    // Get presigned URL from your backend (secure)
    const presignRes = await fetch(`${BACKEND_API_ENDPOINT}/api/v1/file/presign?key=${encodeURIComponent(key)}`, {
        method: 'GET',
        headers: {
